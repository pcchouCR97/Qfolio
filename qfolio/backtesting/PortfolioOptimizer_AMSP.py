...
class PortfolioOptimizer():
    ...
    def optimize(self,
        ...
        else: num_bits = estimate_num_bits_basek(self.B, prices, k = self.k)
        ...
        for i in range(len(crps)):
            ...
            lots, AMSP = AMSP_estimator(latest_prices, 
            ...
            print("\n" + "="*20 + f" DEBUG FOR PERIOD {crp} " + "="*20)
            print(f"Assets: {self.assets}")
            print(f"Calculated Expected Returns (r_i): {r_i}")
            print("="*60 + "\n")
            # --- END DEBUGGING CODE ---
            sys.exit("test size")
            """
            """
            # --- Temporary code for diagnostics ---
            # Create a sample 'typical' portfolio, e.g., 127 lots spread evenly
            s_sample = np.full(3, 127.0 /3)

            # Calculate the magnitude of the risk term for this sample
            risk_magnitude = s_sample.T @ sigma @ s_sample # sigma is your sigma_adj

            print(f"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            print(f"DIAGNOSTIC: Estimated Risk Magnitude is: {risk_magnitude}")
            print(f"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            ...
            share_allocation_classic = decode_results_AMSP(result=opt_results,
                                                                assets=self.assets, 
                                                                num_bits = num_bits, 
                                                                crp=crp,
            ...
            #print(share_allocation_classic)
