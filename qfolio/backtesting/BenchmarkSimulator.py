...
import sys
import os
from qfolio.utils.utilities import save_files
...
class BenchmarkSimulator():
    ...
    def simulate(self, start_date, end_date, freq='21B', save = False, save_path = None):
        ...
        print("Valid business dates for benchmarking:")
        print(valid_dates)
        ...
        print("Current rebalance points (crps):")
        print(crps)

        for crp, nrp in zip(crps, nrps):
            for a in self.assets:
                lv, ls = bm_val[a][-1], bm_shares[a][-1]
                # Calculate portfolio value at nrp BEFORE adding new investment
                val_at_nrp = ls * self.data[a].loc[nrp]
                bm_val[a].append(val_at_nrp)
                # Update shares for next period by adding new investment AFTER recording value
...
class BenchmarkSimulatorV2():
    """
    Add return value at each rebalance date
    to compare with portfolio return
    """
    ...
    def simulate(self, start_date, end_date, freq='21B', save = False, save_path = None):
        ...
        return df_benchmarks_returns, df_benchmarks
...
def test_benchmark_simulator():
    ...
    returns, vals = sim.simulate(start_date='2025-01-01', end_date='2025-01-31', freq='5B')
