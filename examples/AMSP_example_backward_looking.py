"""
Quantum Portfolio Optimization (AMSP method)

METHODOLOGY:
- Pure systematic strategy with ZERO discretionary decisions
- Portfolio stays fully invested (benchmarking: SPY)
- Risk managed through optimization (q parameter), NO cash-out
- CVaR calculated for monitoring/analysis only
- Comparable to passive buy-and-hold benchmark
- No Risk action triggers trades
...
# backtest period
sim_start_date = "2020-01-03" # Q3 2020 start
sim_end_date = "2025-10-01" # End on October 28, 2025
...
#TODO SamplerVQE with maxiter = 100 get very close to calssical results, this can be the reason of the VQE is specialized to the diagonal Hamiltonians!

# right now, only QAOA_shots (512shots gives a good result), classic, QAOA, SCIP and exact_miqp are stable enough.
...
# **BACKWARD CALCULATION** - Calculate rebalance dates from END date to ensure Oct 28 is a rebalance
print(f"\n**BACKWARD REBALANCE CALCULATION**: Ensuring {sim_end_date} is a rebalance date")
target_ts = pd.Timestamp(sim_end_date)
target_idx = all_trading_days.get_loc(target_ts)
rebal_freq_num = int(rebalance_freq.replace('B', ''))

# Get sim_start_date index to use as boundary
sim_start_ts = pd.Timestamp(sim_start_date)
sim_start_idx = all_trading_days.get_loc(sim_start_ts)

rebalance_indices_list = [target_idx]
current_idx = target_idx
while True:
    prev_idx = current_idx - rebal_freq_num
    if prev_idx < 0 or prev_idx < sim_start_idx: # Stop at sim_start boundary!
        break
    rebalance_indices_list.append(prev_idx)
    current_idx = prev_idx
rebalance_indices = sorted(rebalance_indices_list)
actual_rebalance_dates = all_trading_days[rebalance_indices]
print(f"Calculated {len(actual_rebalance_dates)} rebalances working BACKWARD from {sim_end_date}")
print(f"[OK] Last rebalance: {actual_rebalance_dates[-1].date()} \n")
...
print("[DEBUG] actual_rebalance_dates:", actual_rebalance_dates[0])
print("[DEBUG] Simulation start date for SPY benchmark:", sim_start_date)
#sys.exit("[DEBUG] Exiting after printing rebalance dates.")
...
spy_daily_values, spy_daily_roi = spy_benchmark.simulate(
    start_date=actual_rebalance_dates[0].strftime('%Y-%m-%d'),
...
# Align indices between portfolio and SPY returns
common_dates = opt_daily_returns.index.intersection(spy_daily_returns.index)
opt_daily_returns_aligned = opt_daily_returns.loc[common_dates]
spy_daily_returns_aligned = spy_daily_returns.loc[common_dates]

market_up_days = spy_daily_returns_aligned > 0
market_down_days = spy_daily_returns_aligned < 0

if market_up_days.sum() > 0:
    opt_returns_market_up = opt_daily_returns_aligned[market_up_days].mean() * 252
    spy_returns_market_up = spy_daily_returns_aligned[market_up_days].mean() * 252
...
if market_down_days.sum() > 0:
    opt_returns_market_down = opt_daily_returns_aligned[market_down_days].mean() * 252
    spy_returns_market_down = spy_daily_returns_aligned[market_down_days].mean() * 252
...
common_dates_plot = opt_res_df.index.intersection(spy_values.index)
opt_res_df_aligned = opt_res_df.loc[common_dates_plot]
spy_values_aligned = spy_values.loc[common_dates_plot]
opt_beats_spy = opt_res_df_aligned > spy_values_aligned
if opt_beats_spy.any():
    plt.fill_between(opt_res_df_aligned.index, opt_res_df_aligned, spy_values_aligned,
...
# Save composition history to CSV
if composition_history:
    comp_records = []
...
# Save sector exposure history to CSV
if sector_exposure_history:
    sector_df = pd.DataFrame(sector_exposure_history)
...
# Save concentration history to CSV
if concentration_history:
    conc_df = pd.DataFrame(concentration_history)
...
# Generate sector frequency summary table
if sector_exposure_history:
    sector_frequency = {}
    sector_total_weight = {}
