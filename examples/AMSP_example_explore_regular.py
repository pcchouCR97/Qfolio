"""
Parameter Space Exploration for Regular (Non-Hybrid) Quantum Portfolio Optimization
...
Purpose: Find optimal (q, Budget) configurations independently for the regular method
before comparing with hybrid method.

Runtime: ~18-20 hours (20-25 min per backtest)

Usage:
    python AMSP_explore_regular.py

Output:
    results/results_regular_q*.json (56 files)
    logs/exploration_regular_log.csv
"""
...
K = 2
H_SCALE = 1e5
SOLVER_TYPE = 'QAOA' # classic or QAOA
SHARPE_N = 3
REBALANCE_FREQ = '63B'
...
def run_single_backtest(q, budget, run_number, total_runs):
    ...
    """
    print(f"\n{'='*90}")
    print(f"RUN {run_number}/{total_runs}: REGULAR METHOD")
...
def print_summary(log_entries, total_duration_sec):
    ...
    print(f"\n{'='*90}")
    print(f"EXPLORATION SUMMARY: REGULAR METHOD")
    print(f"{'='*90}")
    ...
    print(f"Log File:          {os.path.abspath(os.path.join(LOGS_DIR, f'exploration_regular_{SOLVER_TYPE}_log.csv'))}")
    print(f"{'='*90}\n")
...
def main():
    ...
    # Print header
    print(f"\n{'='*90}")
    print(f"PARAMETER SPACE EXPLORATION: REGULAR METHOD")
    print(f"{'='*90}")
    print(f"q values:      {len(Q_VALUES)} levels: {Q_VALUES}")
    print(f"Budgets:       {len(BUDGETS)} levels: {BUDGETS}")
    print(f"Total Runs:    {total_runs}")
    print(f"Est. Time:     {total_runs * 2 / 60:.1f} hours (@ 2 min/run)")
    ...
    run_number = 0
    start_time = time.time()
    log_path = os.path.join(LOGS_DIR, f'exploration_regular_{SOLVER_TYPE}_log.csv')
